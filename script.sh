...
setprop init.svc_debug.no_fatal.vendor.qti.hardware.display.composer true
...
chroot /data/local/debian /usr/bin/env PATH=/bin:/usr/bin:/sbin:/usr/sbin TMPDIR=/tmp /bin/bash -l -c "
    ...
    # === GRAFİK MOTORU (SABİT CPU RENDER) ===
    ...
    # Masaüstü açıldıktan 3 saniye sonra ekranı yatay yap
    (
        sleep 3
        export XDG_RUNTIME_DIR=/tmp/runtime
